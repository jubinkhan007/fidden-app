...
class _BookingSummaryScreenState extends State<BookingSummaryScreen> {
  ...
  Future<void> _chooseCoupon() async {
    ...
    final serviceId =
        booking['service_id'] as int? ?? args['serviceId'] as int? ?? 0;
    ...
    final res = await Get.toNamed(
      '/select-coupon',
      arguments: {'shopId': shopId, 'serviceId': serviceId},
    );
  ...
  bool _isSelectedSlotInFuture({
    ...
    final slots =
        (preload['slots'] as List?)?.cast<Map<String, dynamic>>() ?? const [];
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        final realBookingId =
            controller.paymentBookingId.value; // 0 until paymentIntent returns
        if (realBookingId > 0) {
          ...
          child: Column(
            ...
            children: [
                ...
                Card(
                  ...
                  child: ListTile(
                    leading: Container(
                      ...
                      child: Text(
                        _appliedCoupon!.shortAmountLabel,
              ...
              if (serviceNiche == 'tattoo_artist' ||
                ...
                Obx(() {
                  final data = controller.designRequestData;
                  final hasData =
                      data.isNotEmpty &&
                  ...
                  if (!hasData) {
                    return SizedBox(
                      ...
                      child: OutlinedButton.icon(
                        ...
                        label: const Text("Add Design Request"),
                        style: OutlinedButton.styleFrom(
                        ...
                        onPressed: () async {
                          ...
                          if (result != null && result is Map) {
                            controller.designRequestData.value =
                ...
                () => Column(
                  children: [
                    ...
                    _PaymentOptionTile(
                      title: 'PayPal',
                      icon: Icons
                          .paypal, // Or use Icons.payment if paypal icon isn't available
                      value: 'paypal',
                      groupValue: controller.selectedPaymentMethod.value,
                      onChanged: (v) => controller.setPaymentMethod(v!),
                    ),
                ...
                child: Column(
                  children: [
                    ...
                    if (_appliedCoupon != null && _couponDiscount > 0)
                      _PriceRow(
                        label:
                            'Coupon Discount (${_appliedCoupon!.inPercentage ? '${_appliedCoupon!.amount.toInt()}%' : ''})',
                    ...
                    if (discountPrice != null && _appliedCoupon == null)
                      _PriceRow(
                        label: 'Discount',
                        amount:
                            '-${(servicePrice - (double.tryParse(discountPrice.toString()) ?? 0)).toStringAsFixed(2)}',
                        isDiscount: true,
                      ),
              ...
              Obx(() {
                ...
                DateTime? start;
                try {
                  start = _slotFmt.parse(selectedSlot).toLocal();
                } catch (_) {}
                String extra = '';
                if (start != null) {
                  final freeDeadline = start.subtract(Duration(hours: p.freeH));
                  extra =
                      ' (until ${DateFormat('MMM d, h:mm a').format(freeDeadline)})';
                }

                final parts = <String>[
                  'Free cancellation up to ${p.freeH}h$extra.',
                  if (p.feePct > 0)
                    'A ${p.feePct}% fee may apply within ${p.freeH}h.',
                  if (p.noRefundH > 0)
                    'No refunds within the last ${p.noRefundH}h.',
                ];
                    ...
                    ? () {
                        ...
                        if (!_isSelectedSlotInFuture(
                          ...
                          Get.defaultDialog(
                            ...
                            middleText:
                                'Please pick another time. We’ll find you a new opening.',
                            textConfirm: 'OK',
                            onConfirm: Get.back,
                          );
                          return;
                        ...
                        // NEW: Pass add_on_ids
                        final rawAddOns =
                            (Get.arguments as Map?)?['add_on_ids'];
  ...
  Future<void> _openScheduleSheet() async {
    ...
    final c = Get.isRegistered<ServiceDetailsController>(tag: tag)
        ...
        : Get.put(
            ServiceDetailsController(serviceId),
            tag: tag,
            permanent: true,
          );
    ...
    c.fetchSlotsForDate(dayToLoad, useCache: true).then((_) {
      if (initLocal != null) {
        for (final s in c.slots) {
          final diff = s.startTimeUtc
              .toLocal()
              .difference(initLocal)
              .inMinutes
              .abs();
          if (diff <= 1) {
