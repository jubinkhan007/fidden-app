      ...
      UserBookingsModel(
        ...
        results: List<BookingItem>.from(
          (json["results"] as List<dynamic>? ?? []).map(
            (x) => BookingItem.fromJson(x as Map<String, dynamic>),
          ),
        ),
      );
...
class AddOnService {
  ...
  AddOnService({required this.title, required this.duration});
  ...
  Map<String, dynamic> toJson() => {'title': title, 'duration': duration};
}
...
class BookingItem {
  ...
  final String? serviceImg; // Service image URL
  final int slot;
